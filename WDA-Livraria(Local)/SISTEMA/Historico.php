...
if (!empty($_GET['search'])) {
    ...
    AND (livro.Nome_livro LIKE '%$search%' 
         OR usuario.Nome_usuario LIKE '%$search%' 
         OR histórico.Data_aluguel LIKE '%$search%' 
         OR histórico.Data_previsão LIKE '%$search%' 
         OR histórico.Data_devolução LIKE '%$search%'
         OR Status_hist LIKE '%$search%')";
} else {
    $sqlhist = "SELECT histórico.Cod_hist, livro.Nome_livro, usuario.Nome_usuario, histórico.Data_aluguel, histórico.Data_previsão, histórico.Data_devolução, histórico.Quantidade
    FROM histórico, livro, usuario 
    WHERE histórico.Livro = livro.Cod_livro 
            ...
            <tbody>
                ...
                while ($hist_data = mysqli_fetch_assoc($resultadoConsulta)) {
                    ...
                    if (strtotime($hist_data['Data_previsão']) >= strtotime($hist_data['Data_devolução'])) {
                        $sqlstatus = "UPDATE histórico SET Status_hist = 'Livro no Prazo' WHERE Cod_hist = $hist_data[Cod_hist]";
                        ...
                        $sqlstatus = "UPDATE histórico SET Status_hist = 'Livro fora Prazo' WHERE Cod_hist = $hist_data[Cod_hist]";
                        $status = $conexao->query($sqlstatus);
                        echo "<td>
