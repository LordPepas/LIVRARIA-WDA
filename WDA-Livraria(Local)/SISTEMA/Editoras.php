...
if (!empty($_GET['search'])) {
    ...
    WHERE Cod_editora LIKE '%$search%' or Nome_editora 
    LIKE '%$search%' or Cidade LIKE '%$search%' or Contato 
    LIKE '%$search%'";
} else {
...
$orderColumn = isset($_GET['order']) ? limparEntrada($_GET['order']) : 'Cod_editora';
...
$resultadoTotal = executarConsulta($conexao, $sqleditora);
...
if (!empty($search)) {
    $sqleditora .= " ORDER BY Cod_editora DESC";
} else {
    $sqleditora .= $orderClause;
    $sqleditora .= " LIMIT $registrosPorPagina OFFSET $offset";
}
// Adiciona o limite e offset na consulta SQL

// Executa a consulta SQL
$resultadoConsulta = executarConsulta($conexao, $sqleditora);
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="table-responsive">
        <div class="header-table d-flex justify-content-between align-items-center">
            ...
            <a href="../CREATE/C_editora.php" class="btn btn-success">ADICIONAR+</a><br>
            <!-- Campo de busca -->
        ...
        <table class="table text-white table-bg">
            <thead class="table-header">
                <tr>
                    <th><a class="header" href="?order=Cod_editora&direction=<?php echo ($orderColumn === 'Cod_editora' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">ID</a></th>
                    <th><a class="header" href="?order=Nome_editora&direction=<?php echo ($orderColumn === 'Nome_editora' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Nome</a></th>
                    ...
                    <th><a class="header" href="?order=Contato&direction=<?php echo ($orderColumn === 'Contato' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Contato</a></th>
                    <th>Ações</th>
                </tr>
            </thead>
            ...
            <tbody>
                ...
                while ($editora_data = mysqli_fetch_assoc($resultadoConsulta)) {
        ...
        <div class="pagination <?php if (!empty($search)) echo 'd-none'; ?>">
            ...
            <ul class="pagination">
                <li class="page-item <?php echo ($paginaAtual == 1) ? 'disabled' : ''; ?>">
                    <a class="page-link" href="Editoras.php?order=<?php echo $orderColumn; ?>&direction=<?php echo $orderDirection; ?>&pagina=<?php echo ($paginaAtual - 1); ?>" aria-label="Anterior">
                ...
                if ($paginaAtual > 3) {
                    echo "<li class='page-item'><a class='page-link' href='Editoras.php?order=$orderColumn&direction=$orderDirection&pagina=1'>1</a></li>";
                    echo "<li class='page-item disabled'><span class='page-link'>...</span></li>";
                }

                // Exibir páginas anteriores à página atual
                for ($i = max(1, $paginaAtual - 1); $i < $paginaAtual; $i++) {
                    echo "<li class='page-item'><a class='page-link' href='Editoras.php?order=$orderColumn&direction=$orderDirection&pagina=$i'>$i</a></li>";
                }
                ...
                if ($paginaAtual < $totalPaginas - 2) {
                    echo "<li class='page-item disabled'><span class='page-link'>...</span></li>";
                    echo "<li class='page-item'><a class='page-link' href='Editoras.php?order=$orderColumn&direction=$orderDirection&pagina=$totalPaginas'>$totalPaginas</a></li>";
                }
                ?>
                <li class="page-item <?php echo ($paginaAtual == $totalPaginas) ? 'disabled' : ''; ?>">
                    <a class="page-link" href="Editoras.php?order=<?php echo $orderColumn; ?>&direction=<?php echo $orderDirection; ?>&pagina=<?php echo ($paginaAtual + 1); ?>" aria-label="Próxima">
