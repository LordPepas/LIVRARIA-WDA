...
if (!empty($_GET['search'])) {
    ...
    $sqluser = "SELECT * FROM usuario WHERE Cod_usuario LIKE '%$search%' or Nome_usuario LIKE '%$search%' or Cidade LIKE '%$search%' or Endereço LIKE '%$search%' or Email LIKE '%$search%'";
} else {
    $sqluser = "SELECT * FROM usuario";
}

// Verifica qual cabeçalho foi clicado para ordenação
$orderColumn = isset($_GET['order']) ? limparEntrada($_GET['order']) : 'Cod_usuario';
...
function executarConsulta($conexao, $sqluser)
{
    $result = mysqli_query($conexao, $sqluser);
    if ($result === false) {
...
$resultadoTotal = executarConsulta($conexao, $sqluser);
...
if (!empty($_GET['search'])) {
    $sqluser .= " ORDER BY Cod_usuario DESC";
} else {
    $sqluser .= $orderClause;
    $sqluser .= " LIMIT $registrosPorPagina OFFSET $offset";
}
// Executa a consulta SQL
$resultadoConsulta = executarConsulta($conexao, $sqluser);
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarText">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="Usuarios.php" style="text-decoration:underline">CLIENTES</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="table-responsive">
        <div class="header-table d-flex justify-content-between align-items-center">
            ...
            <a href="../CREATE/C_usuario.php" class="btn btn-success">ADICIONAR+</a><br>
            <!-- Campo de busca -->
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <table class="table text-white table-bg">
                <thead class="table-header">
                    ...
                    <tr>
                        <th><a class="header" href="?order=Cod_usuario&direction=<?php echo ($orderColumn === 'Cod_usuario' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">ID</a></th>
                        <th><a class="header" href="?order=Nome_usuario&direction=<?php echo ($orderColumn === 'Nome_usuario' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Nome</a></th>
                        <th><a class="header" href="?order=Cidade&direction=<?php echo ($orderColumn === 'Cidade' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Cidade</a></th>
                        <th><a class="header" href="?order=Endereço&direction=<?php echo ($orderColumn === 'Endereço' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Endereço</a></th>
                        <th><a class="header" href="?order=Email&direction=<?php echo ($orderColumn === 'Email' && $orderDirection === 'ASC') ? 'desc' : 'asc'; ?>&pagina=<?php echo $paginaAtual ?>">Email</a></th>
                        <th>Ações</th>
                    </tr>
                ...
                <tbody id="table-body">
                    ...
                    while ($user_data = mysqli_fetch_assoc($resultadoConsulta)) {
        ...
        <div class="pagination <?php if (!empty($search)) echo 'd-none'; ?>">
            ...
            <ul class="pagination">
                <li class="page-item <?php echo ($paginaAtual == 1) ? 'disabled' : ''; ?>">
                    <a class="page-link" href="Usuarios.php?order=<?php echo $orderColumn; ?>&direction=<?php echo $orderDirection; ?>&pagina=<?php echo ($paginaAtual - 1); ?>" aria-label="Anterior">
                ...
                if ($paginaAtual > 3) {
                    echo "<li class='page-item'><a class='page-link' href='Usuarios.php?order=$orderColumn&direction=$orderDirection&pagina=1'>1</a></li>";
                    echo "<li class='page-item disabled'><span class='page-link'>...</span></li>";
                }

                // Exibir páginas anteriores à página atual
                for ($i = max(1, $paginaAtual - 1); $i < $paginaAtual; $i++) {
                    echo "<li class='page-item'><a class='page-link' href='Usuarios.php?order=$orderColumn&direction=$orderDirection&pagina=$i'>$i</a></li>";
                }
                ...
                if ($paginaAtual < $totalPaginas - 2) {
                    echo "<li class='page-item disabled'><span class='page-link'>...</span></li>";
                    echo "<li class='page-item'><a class='page-link' href='Usuarios.php?order=$orderColumn&direction=$orderDirection&pagina=$totalPaginas'>$totalPaginas</a></li>";
                }
                ?>
                <li class="page-item <?php echo ($paginaAtual == $totalPaginas) ? 'disabled' : ''; ?>">
                    <a class="page-link" href="Usuarios.php?order=<?php echo $orderColumn; ?>&direction=<?php echo $orderDirection; ?>&pagina=<?php echo ($paginaAtual + 1); ?>" aria-label="Próxima">
